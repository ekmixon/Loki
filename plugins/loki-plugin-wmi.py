            ...
            try:
                ...
                if hashEntry not in knownHashes:
                    logger.log(
                        "WARNING",
                        "WMIScan",
                        f"CLASS: __eventFilter MD5: {hashEntry} NAME: {eventFilter.wmi_property('Name').value} QUERY: {eventFilter.wmi_property('Query').value}",
                    )
            ...
            try:
                ...
                if hashEntry not in knownHashes:
                    logger.log(
                        "WARNING",
                        "WMIScan",
                        f"CLASS: __FilterToConsumerBinding MD5: {hashEntry} CONSUMER: {FilterToConsumerBinding.wmi_property('Consumer').value} FILTER: {FilterToConsumerBinding.wmi_property('Filter').value}",
                    )
            ...
            try:
                ...
                if hashEntry not in knownHashes:
                    logger.log(
                        "WARNING",
                        "WMIScan",
                        f"CLASS: CommandLineEventConsumer MD5: {hashEntry} NAME: {CommandLineEventConsumer.wmi_property('Name').value} COMMANDLINETEMPLATE: {CommandLineEventConsumer.wmi_property('CommandLineTemplate').value}",
                    )
